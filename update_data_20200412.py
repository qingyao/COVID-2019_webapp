...
prev_read_date = set([i.split('_')[-1].replace('.json','') for i in all_files])
...
new_data_sorted = sorted(os.listdir('new_data'))
for new_data in new_data_sorted:
    new_data_date = new_data.split('_')[-1].replace('.txt','')
    if new_data_date in prev_read_date or not(new_data_date.startswith('04')):
        continue
    with open('new_data/{}'.format(new_data)) as f:
        content = f.read()
    print(new_data_date) 
